...
**Task** In the console (look below), type 2+2 and hit enter. What does it return? Type your answer below. **Answer: It returns the number 4.**
...
**Answer: It returns the number 4.**
...
**Answer: The number of observations in 79,366 and the number of variables is 9.**
...
**Answer: The name of the business from the first row of the "name" column is Directed Research.**
...
**Answer: I believe that the first line that is group_by(term), the second and third lines are then summarizing the data in the count_classes column which is counting the number of courses offered per semester.**
...
**Answer: It seems that what has changed is the order in which the data is shown. In this example, it seems to be showing the data in a descending order with the descending being via the terms, or semesters.**
...
**Answer: The output of the code has significantly changed as now the department has been added to the data, showcasing how many classes are offered in each department each term, or semester. This was changed by adding the department column to the group_by and arrange parts of the code.**
...
**Answer: What has changed in the code was the addition of the total_seats column in the code, which uses the sum of the seats column. Additionally, the arrange() part of the code has changed, now focusing on arranging the number of total seats in descending order. This new code has created a new column titled total_seats, and the data is organized by the number of total seats available for a semester of classes all over campus in descending order.**
...
**Answer: New functions have been added that are intended to calculate the mean and median of seats in classes. This turns out two new columns in the data, mean_seats and median_seats. The top terms are dealing with the number of seats and/or the number of students that are at the university during a given semester.**
...
**Answer: The main thing that changed was the added column focusing on the department, shifting the focus from by term or semester to department and the amount of classes that are given in total, matched with the total number of seats that have been offered, and the mean and median of seats.**
...
1.  Has there been a change in the amount of seats being offered per department over time, say by semester?

2.  Have there been any major changes in the median amount of seats for certain departments over time?

3.  Has a change been seen in the amount of classes offered per department over time? Have some departments been increasing the number of classes offered while some had decreased the number of classes offered per term/semester?
