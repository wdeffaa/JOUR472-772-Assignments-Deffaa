...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? Where is that zip code, and does that zip code make sense to you as the top recipient? Write a sentence describing the top result. **While the top result is 21202, which is Baltimore, Maryland. The area code 20001 is up near the top the most times, this area code is Washington D.C., which makes sense because D.C. is visited very often by tourists, therefore making near the top of the list for most years.**
