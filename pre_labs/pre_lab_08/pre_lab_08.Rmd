...
**Task** Run the following code to load the data. In the space below, indicate how many rows and columns it has. **We have 3822 Rows and 4 Columns.**
...
**The first line creates a new table; the mutate line makes the months be represented as letters such as "Jan" rather than numbers; then we group by the month and summarize the total calls, and then arrange them in descending order.**
...
**Just a simple blank, grey canvas.**
...
**The changed codeblock has made the bar chart in an ascending order in terms of count.**
...
**Now, the bar chart has been flipped horizontally.**
...
**The background is now just a plain white background.**
...
**Now, the background is blue as opposed to a previous white background.**
...
**Now there is a proper title and subheading at the bottom right, showing the source.**
...
**What I see is the number of total calls per day on every single day of the year in 2022.**
...
**I see a crazy line chart that takes the previous code information and makes it into a line chart.**
...
**Now it seems to be that there is a lot of labels, but they all seem to be overlapping each other so it is very confusing and can be barely read.**
...
**This now puts the names horizontally, but it is still quite hard to read.**
