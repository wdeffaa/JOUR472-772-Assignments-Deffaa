...
**This code looks like it is adding other columns that look to possibly be adding other candidates that ran for governor and the amount of votes they got.**
...
**The code now changed the decimals into more readable percentages and ordered them in descending order, showing us which counties had the largest percentage of votes toward Wes Moore.**
...
**Yes because this puts the counties that votes him the least at the top, arguably a more important statistic that can be made into a story.**
...
1.  **str_to_sentence, which converts a string to a sentence case, capitalizing the first letter of the first word in a sentence.**
2.  **str_to_title, which converts a string to a title case, capitalizing the first letter of every word, as it it were in a title.**
...
**I think the mutate statement is having RStudio detect the name/abbreviation of certain states, as to find out how much expenses are coming from each of the states named in the code.**
...
**Yes it does, it clearly shows which state is contributing the most in expenses toward the Wes Moore campaign.**
...
1.  **What was the most popular expense method in each state?**

2.  **What was the most popular expense category by state?**

3.  **Did a singular company donate to the campaign from multiple different states (a national company perhaps?)**
