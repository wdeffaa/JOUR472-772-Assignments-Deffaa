...
```{r}
pg_county_overdoeses <- read_csv("data/prince_georges_2022_overdoses.csv") %>% clean_names()
```

You will need to create columns for the date, week and month based on the existing `datetime` column.

```{r}
pg_county_overdoses_date <- pg_county_overdoeses %>%
...
**Yes, there are some noticeable differences in this bar chart than the one in pre-lab 8. The highest and lowest months are different than the ones in the pre-lab, with the lowest month now being Jan. and the highest month now being Dec.**

```{r}
pg_calls_permonth <- pg_county_overdoses_date %>%
  group_by(month) %>%
...
pg_calls_permonth %>%
  ...
  labs(
    title="Prince Georges County Calls Per-Month",
    x = "Month",
    y = "Total Calls",
...
**For the most part there is a big up and down in terms of calls made, but there is a very noticeable spike during the middle of the year, just before the 30 week mark. Additionally, there is another smaller spike right before the end of the year.**

```{r}
pg_calls_perweek <-  pg_county_overdoses_date %>%
  group_by(week) %>%
  summarise(total_calls = n())
...
**There is a pretty large pattern between zipcodes with lower income having more calls, but at the same time, even lower income zip codes have low call numbers. This would be an interesting point to investigate.**

```{r}

pg_calls_zip <- pg_county_overdoeses %>%
  group_by(zipcode) %>%
  summarise(total_calls = n())
...
ggplot(pg_calls_with_income, aes(x = estimate, y = total_calls, label = zipcode)) +
  geom_point(color = "black", size = 3, alpha = 1)
  labs(
    title = "Total 911 Overdose Calls vs. Median Household Income by ZIP Code",
    subtitle = "Prince George’s County, Maryland (2019)",
    x = "Median Household Income (USD)",
    y = "Total 911 Overdose Calls",
    caption = "Sources: Prince George’s County 911 Calls; U.S. Census Bureau, American Community Survey 2019"
  ) +
  theme_minimal() +
  theme(
